...
物理名はcamelCaseで記載する。DBカラムと直接紐づくことが前提の項目に関しては、DBの物理名(snake_caseで定義されている)をcamelCaseに置き換えたものを物理名とする。また、紐づき先のテーブル・カラムを論理名で定義する。
...
| 取得件数   | limit       | number | -          | -        | 任意 | -        | -        | -            | 1      | 100    | ページネーション: 1回の取得上限件数(デフォルト: 100)   |
| オフセット | offset      | number | -          | -        | 任意 | -        | -        | -            | 0      | -      | ページネーション: 取得開始位置                         |
