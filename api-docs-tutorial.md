...
- アクションベース (機能/コレクション全体): `/api/v1/{リソース名}/<アクション>`
  - 認証・メール送信などの機能コンテキスト、またはコレクション全体に対する操作(例: list)に用いる。

- リソースベース (登録操作): `/api/v1/{リソース名}`
  - POSTメソッドを用いて、新規リソースの登録操作に用いる。

- リソースベース (個別リソース操作): `/api/v1/{リソース名}/{id}`
  - GET/PATCH/PUT/DELETEメソッドを用いて、特定IDに対する個別取得、更新、削除など、単一リソースのRUD操作に用いる。

- ベースパス: `/api/v1`固定(v2へのバージョンアップといった対応は本書の対象外とする)。
- リソース名: DB定義書に従ったテーブル物理名(kebab-caseに変換)もしくはドメインコンテキスト(機能モジュール)を表現する名詞。
- アクション: 認証・メール送信などの機能コンテキスト、または一覧取得(list)など、コレクション全体に対する機能実行。
- id: リソースベースの構造は、特定IDに対する個別取得(detail)、更新(update)、削除(delete)など、単一リソースのCRUD操作に用いる。

- パスパラメータは、`.../{parameterA}/{parameterB}`のように、`{}`で物理名を示すことで定義する。本書ではNestJSによる実装を前提としており、実装時には`:parameterA`のようにコロン付き表記に読み替える想定である。
- URLパラメータは、`...?{parameterA: string}&{parameterB: number}&{parameterC: date}`のように、エンドポイントの末尾に`?`に続く`{}`で物理名と型を示すことで定義する。
...
API個別設計書ファイル名は、`{リソース名}_<アクション>.md`の形式で命名する。視認性のための順番管理はAPI一覧で行う。「アクション」の詳細は「API一覧」に記載している。
...
| ユーザー作成 | `/api/v1/users/`         | `users_create.md`     | ユーザーテーブルに紐づくアクションのため、テーブル物理名がリソースとなる。         |
...
#### 1.2.1. POSTによる取得処理の許可

- **目的:** 下記の目的のため、取得系APIをPOSTで設計することを可とする。
  - 日本国内のエンタープライズシステム設計の慣習に基づくURLパラメータからの簡易的な情報隠蔽措置
  - 複雑な検索条件により、URLの文字数制限を超過することを防止するため
...
| 形式 | エンドポイント名(例) | 役割                            |
| ---- | -------------------- | ------------------------------- |
| 登録 | `/api/v1/users/`     | 対象テーブルに1レコード登録する |
...
| 形式 | エンドポイント名(例) | 役割                                |
| ---- | -------------------- | ----------------------------------- |
| 更新 | `/api/v1/users/{id}` | 1レコードの特定フィールドを更新する |

#### 1.4.1. PATCH と PUT の使い分け

更新系APIでは、以下のポリシーで PATCH と PUT を使い分ける。
...
| No  | API名(個別設計書へのリンク)           | エンドポイント       | リソース | アクション | メソッド      | ビジネスドメイン | 認証要否 | 備考                                                                             |
| --- | ------------------------------------- | -------------------- | -------- | ---------- | ------------- | ---------------- | -------- | -------------------------------------------------------------------------------- |
| 101 | [ログイン](./auth_login.md)           | `/api/v1/auth/login` | `auth`   | `login`    | `POST`        | 認証             | 不要     |                                                                                  |
| 201 | [ユーザー一覧取得](./users_list.md)   | `/api/v1/users/list` | `users`  | `list`     | `POST`        | ユーザー管理     | 要       | 検索条件による一覧検索。ページネーション・ソートはリクエストボディに含める想定。 |
| 202 | [ユーザー詳細取得](./users_detail.md) | `/api/v1/users/{id}` | `users`  | `detail`   | `GET`         | ユーザー管理     | 要       | IDによる単一検索。                                                               |
| 203 | [ユーザー情報登録](./users_create.md) | `/api/v1/users/`     | `users`  | `create`   | `POST`        | ユーザー管理     | 要       | -                                                                                |
| 204 | [ユーザー情報更新](./users_update.md) | `/api/v1/users/{id}` | `users`  | `update`   | `PATCH`/`PUT` | ユーザー管理     | 要       | 原則PATCHを選択。リソース洗い替えを意図する場合のみPUTを用いる。                 |
| 205 | [ユーザー情報削除](./users_delete.md) | `/api/v1/users/{id}` | `users`  | `delete`   | `DELETE`      | ユーザー管理     | 要       | 物理削除(画面提供しない想定だが、サンプルとして記載)                             |

- Noは一意に記載すること。順番管理に用いるため、トビを許容する。

- アクションは、リソースの種類によって下記のように設定する。
  - エンティティの場合: CRUDの種別により、下記の通り設定する。
    - `list` 検索条件を指定して、一覧形式で取得する。
    - `detail` IDを指定して単一形式で取得する。
    - `create` リソースを登録する。
    - `update` リソースを更新する。論理削除も含む。
    - `delete` リソースを物理削除する。
  - 機能コンテキストの場合: `login` `send`など、具体的なアクション名を設定する。

- ビジネスドメインは、各APIが属する業務上の区分を記載する。
...
| リソース         | 対象とするエンティティや機能コンテキスト                                                                        |
| アクション       | APIの動作                                                                                                       |
| メソッド         | HTTPメソッド (GET, POST, PATCH, PUT, DELETE)                                                                    |
| ビジネスドメイン | APIが属する業務上の区分                                                                                         |
| 認証要否         | 要/不要                                                                                                         |
...
- 型はJavascriptの型(string, number, date, boolean, object, array)で記載する。
  - number型の場合、フォーマット欄に整数型(`int`)、浮動小数点型(`float`)、10進数型(`decimal(P, S)` `P`は桁数、`S`は精度)の記載を必須とする。
  - string型かつ決まったフォーマットが存在する場合(メールアドレス、電話番号等)、フォーマット欄の記載を推奨する。
仕様上、リクエスト項目がない(空オブジェクト)の場合でも、ヘッダのみ記載すること。
...
| 合計額     | totalAmount | number | -          | -        | 任意 | -        | -        | int          | 1      | 999    | 合計額は動的に計算するためDBに持たない想定             |
| 取得件数   | limit       | number | -          | -        | 任意 | -        | -        | int          | 1      | 100    | ページネーション: 1回の取得上限件数(デフォルト: 100)   |
| オフセット | offset      | number | -          | -        | 任意 | -        | -        | int          | 0      | -      | ページネーション: 取得開始位置                         |
| ソートキー | sortBy      | string | -          | -        | 任意 | -        | -        | -            | -      | -      | ソート: ソート対象の物理名(例: orderDate, totalAmount) |
| ソート順   | sortOrder   | string | -          | -        | 任意 | -        | -        | asc/desc     | -      | -      | ソート: 昇順/降順(デフォルト: desc)                    |
```
...
| 数量       | quantity   | number | 注文       | 注文数   | 必須 | -        | -        | int          | 1      | 999    |                      |
```
...
| 論理名 | 物理名 | 型     | DBテーブル | DBカラム | 必須 | 最小桁数 | 最大桁数 | フォーマット | 最小値 | 最大値 | 備考             |
| ------ | ------ | ------ | ---------- | -------- | ---- | -------- | -------- | ------------ | ------ | ------ | ---------------- |
| 注文ID | id     | string | 注文       | ID       | 必須 | 36       | 36       | UUID         | -      | -      | 更新対象の注文ID |

### ルートオブジェクト(リクエスト)

| 論理名   | 物理名 | 型     | DBテーブル | DBカラム | 必須 | 最小桁数 | 最大桁数 | フォーマット | 最小値 | 最大値 | 備考                       |
| -------- | ------ | ------ | ---------- | -------- | ---- | -------- | -------- | ------------ | ------ | ------ | -------------------------- |
| 注文日   | orderDate| date | 注文       | 注文日   | 必須 | -        | -        | -            | -      | -      | 更新対象の注文日           |
| 注文明細 | items  | array  | -          | -        | 必須 | -        | -        | -            | 0      | 50     | 注文に含まれる明細のリスト |

※ヘッダ部分の更新で明細部分は更新しない場合、注文明細は空配列となる。
...
- セクション名を`### パスパラメータ` あるいは`URLパラメータ`として、リクエストボディ同様に定義する。
- パスパラメータ定義は、エンドポイント定義のパスパラメータ表記と件数・物理名・並び順を完全に一致させ、すべて必須項目として定義すること。
- URLパラメータ定義は、エンドポイント定義のURLパラメータ表記と件数・物理名・型を完全に一致させて定義すること。
- パスパラメータ、URLパラメータとも、URIエンコーディングによる文字数増加も含めた最終的な文字数の合計が2000文字未満に収まるよう留意し、それ以上になることが想定される場合はリクエストボディへの移植を検討せよ。
...
- **0件時のレスポンス**
  ...
  - 個別取得(`/detail`)の場合
    ...
    - **レスポンス** なし。`404 NOT FOUND`を受領した後の処理(エラー表示、空白等)は受領側の責務とする。
...
| 403 FORBIDDEN             | 権限がありません             | -                          | 当該ユーザーにアクセス権がない(認可されていない)場合       |
| 404 NOT FOUND             | 対象のデータが見つかりません | -                          | 個別取得や更新APIで指定IDのデータが存在しない場合          |
